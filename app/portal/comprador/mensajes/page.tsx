    ...
    <div>
      ...
      {conversations.length === 0 ? (
        ...
        <div className="mt-6 space-y-2">
          {conversations.map((c) => {
            const badge = c.artisan.subscriptions?.[0]?.plan;
            return (
              <Link
                key={c.id}
              ...
              >
                {/* Avatar */}
                <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-accent/10 font-serif text-sm font-medium text-accent">
                  {c.artisan.displayName.slice(0, 2).toUpperCase()}
                </div>
