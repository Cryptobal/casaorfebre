...
export function ProductCard({ product, listName, curatorNote }: ProductCardProps) {
  ...
  return (
    <div className="group relative">
      <Link href={`/coleccion/${product.slug}`} className="block" onClick={handleSelectItem}>
        ...
        <div className="relative aspect-[3/4] overflow-hidden rounded-lg bg-background">
          ...
          {product.isCuratorPick && (
            <span className="absolute left-3 top-3 z-10 rounded-full bg-accent/80 px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-white backdrop-blur-sm">
      ...
      {/* Favorite heart — sibling of the link (not nested in <a>) so the markup
          stays valid and keyboard/screen-reader focus is predictable. */}
      <div className="absolute right-3 top-3 z-10 flex items-center gap-1">
        <button
          type="button"
          className={`flex h-9 w-9 items-center justify-center rounded-full bg-surface/80 backdrop-blur-sm transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent ${
        ...
        {product.favoriteCount > 0 && (
          <span className="pointer-events-none rounded-full bg-surface/80 px-1.5 py-0.5 text-[10px] text-text-tertiary backdrop-blur-sm">
