...
export function ReturnImageUpload({ onImagesChange }: ReturnImageUploadProps) {
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      if (file.size > MAX_SIZE) {
        setError("Cada imagen no debe superar 10 MB");
        return;
      }
      ...
      try {
        ...
        const res = await fetch("/api/upload/return", {
          method: "POST",
          body: formData,
        });
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-text-tertiary">
        Agrega hasta {MAX_IMAGES} fotos del producto como evidencia (recomendado)
      </p>
      ...
      {(images.length > 0 || uploadingCount > 0) && (
        <div className="flex flex-wrap gap-2">
          ...
          {Array.from({ length: uploadingCount }).map((_, i) => (
            <div
              key={`uploading-${i}`}
              className="flex h-20 w-20 items-center justify-center rounded-lg border border-dashed border-border"
            >
              <svg
